...
//Construtores
ImageProc::ImageProc(int i, Rect b, Point p1, Point p2, ConfigProcessVideo * config): 
...
void ImageProc::analiseMoment(cv::Mat image) {
	...
	if (hasMotion || isMovementDetectionInProgress()) {
		...
		//#########################################
		//caso passe 4 segundo e meio de video
		//preparar a gravacao do video
		if (getBlockFrameCounter() >= (FRAMES_PER_SECOND * 3)) {
			
			if (getMotionCounter() >= MIN_EVENT_OCORRENCIES_ALLOWED_TO_SAVE){
				setSaveBuffer(true);
				//printf("Alcancou o minimo de imagens detectadas - Total Armazenado:[%d] \n", buffer.size());
				if (buffer.size() >= MAX_MAT_IMAGE_IN_BUFFER){
					//printf("Descarregando buffer e liberando memória\n");
					//Descarrega o buffer para liberar memória
					saveBlockOfVideo();
					buffer.shrink();
				}
	...
	buffer.addFrame(imageToProcess);
	image.release();
...
void ImageProc::saveBlockOfVideo(){
	...
	string videoFileName = getFileName() + ".avi" ;
...
void ImageProc::closeVideWriter(){
